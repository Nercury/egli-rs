...
pub fn num_filtered_configs(display: EGLDisplay, attrib_list: &[EGLint]) -> EglCallResult<i32> {
    ...
    if unsafe {
        ffi::eglChooseConfig(display, attrib_list.as_ptr(), ptr::null_mut(), 0, &mut count)
    } != EGL_TRUE {
...
pub fn get_filtered_configs(display: EGLDisplay, attrib_list: &[EGLint], configs: &mut [EGLConfig]) -> EglCallResult<i32> {
    let mut count: int32_t = 0;
    if unsafe {
        ffi::eglChooseConfig(display, attrib_list.as_ptr(), mem::transmute(configs.as_mut_ptr()), configs.len() as int32_t, &mut count)
    } != EGL_TRUE {
        return Err(EglCallError::ChooseConfig);
    }
    Ok(count as i32)
...
pub fn copy_buffers(display: EGLDisplay, surface: EGLSurface,
    ...
    if unsafe {
        ffi::eglCopyBuffers(display, surface, target)
    } != EGL_TRUE {
...
pub fn get_config_attrib(display: EGLDisplay, config: EGLConfig,
    ...
    if unsafe {
        ffi::eglGetConfigAttrib(display, config, attribute, value)
    } != EGL_TRUE {
...
pub fn get_configs(display: EGLDisplay, configs: &mut [EGLConfig]) -> EglCallResult<i32> {
    let mut count: int32_t = 0;
    if unsafe {
        ffi::eglGetConfigs(display, mem::transmute(configs.as_mut_ptr()), configs.len() as int32_t, &mut count)
    } != EGL_TRUE {
        return Err(EglCallError::GetConfigs);
    }
    Ok(count as i32)
...
pub fn make_current(display: EGLDisplay, draw: EGLSurface,
    ...
    if unsafe {
        ffi::eglMakeCurrent(display, draw, read, ctx)
    } != EGL_TRUE {
...
pub fn query_context(display: EGLDisplay, ctx: EGLContext,
                     attribute: EGLint, value: &mut EGLint) -> EglCallResult<()> {
    if unsafe {
        ffi::eglQueryContext(display, ctx, attribute, value)
    } != EGL_TRUE {
...
pub fn query_surface(display: EGLDisplay, surface: EGLSurface,
                     attribute: EGLint, value: &mut EGLint) -> EglCallResult<()> {
    if unsafe {
        ffi::eglQuerySurface(display, surface, attribute, value)
    } != EGL_TRUE {
...
pub fn surface_attrib(display: EGLDisplay, surface: EGLSurface,
    ...
    if unsafe {
        ffi::eglSurfaceAttrib(display, surface, attribute, value)
    } != EGL_TRUE {
...
pub fn swap_interval(display: EGLDisplay, interval: EGLint) -> EglCallResult<()> {
    if unsafe {
        ffi::eglSwapInterval(display, interval)
    } != EGL_TRUE {
